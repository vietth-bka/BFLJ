...
def do_train_epoch(net, data_iter, optimizer, rank, epoch, train_config, config):
    if rank == 0:
    ...
    for (images, gt_boxes, im_info), step in zip(data_iter, range(0, train_config.iter_per_epoch)):
        ...
        if rank == 0:
            if step % train_config.log_dump_interval == 0:
                ...
                line = 'Epoch:{}, iter:{}, lr:{:.5f}, loss_type:{} loss is {:.4f}.'.format(
                    epoch, step, optimizer.param_groups[0]['lr'], config.loss_type, stastic_total_loss)
                print(line)
                print(outputs)
                fid_log.write(line+'\n')
...
def train_worker(rank, train_config, network, config):
    ...
    if train_config.resume_weights:
        model_file = os.path.join(train_config.model_dir,
            'dump-{}af2.pth'.format(train_config.resume_weights))        
        ...
        print('Resume weights', model_file)
    ...
    data_iter = torch.utils.data.DataLoader(dataset=crowdhuman,
            ...
            shuffle=True)
    print('Loaded CrowdHuman..')
    
    for epoch_id in range(begin_epoch, train_config.total_epoch+1):
        do_train_epoch(net, data_iter, optimizer, rank, epoch_id, train_config, config)
        # exit(-1)

        if rank == 0 and epoch_id >=0:
            #save the model
            fpath = os.path.join(train_config.model_dir,
                'dump-{}af2.pth'.format(epoch_id))
...
def run_train():
    ...
    # parser.add_argument('--loss_type', '-l', default='cons',type=str)
