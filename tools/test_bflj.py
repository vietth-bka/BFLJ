...
from evaluate_lmk import compute_JI, compute_APMR
...
def inference_bfj(config, args, network, model_file, device, dataset, start, end, result_queue, result_queue_match):
    torch.set_default_tensor_type('torch.FloatTensor')
    torch.multiprocessing.set_sharing_strategy('file_system')
    # init model
    net = network()
    ...
    for (image, gt_boxes, im_info, ID, image_id) in data_iter:
        ...
        match_result = match_body_face_bfj_new(args, pred_boxes, pred_emb, image_id)        

        result_dict = dict(ID=ID[0], height=int(im_info[0, -3]), width=int(im_info[0, -2]),
                dtboxes=boxes_dump(pred_boxes, pred_lmks, pred_emb), gtboxes=boxes_dump(gt_boxes))
        result_queue.put_nowait(result_dict)
        result_queue_match.put_nowait(match_result)

def match_body_face_bfj_new(args, pred_boxes, pred_emb, image_id):
    """
    new matching method without landmarks
    """   
    ...
    if wof_flag:
        ...
        match_merge_matrix = np.exp(-match_merge_matrix) #find the maximized pairs
        ...
        ignore_indices = row_ind[matched_vals < args.cut_off] #be careful when len(row_ind) < base_body_boxes.shape[0]
...
def match_body_face_bfj_old(args, pred_boxes, pred_emb, image_id):
    """
    old version of matching body and face
    match a body box with a face box basing on the maximum value in a row of the match_merge_matrix
    """
    keep_body = pred_boxes[:, 7] == 1
    keep_face = pred_boxes[:, 7] == 2
    body_boxes = pred_boxes[keep_body]
    ...
    if wof_flag:
        ...
        # print('match_merge_matrix:', match_merge_matrix)
...
def boxes_dump(boxes, lmks=None, embs=None):
    ...
    if boxes.shape[-1] >= 8: # v2 or v3
        if embs is not None:
            result = [{'box':[round(i, 1) for i in box[:4].tolist()],
                   ...
                   'tag':int(box[7]),
                   'emb':emb.tolist(),
                   'lmk':[round(i, 1) for i in lmk.tolist()]
                   } for box, lmk, emb in zip(boxes, lmks, embs)]
        else:
            result = [{'box':[round(i, 1) for i in box[:4].tolist()],
                    'score':round(float(box[6]), 5),
