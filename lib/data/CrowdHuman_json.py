...
class CrowdHuman(torch.utils.data.Dataset):
    def __init__(self, config, if_train):
        if if_train:
            ...
            self.records = pickle.load(open('../lib/data/CrHu_train15k.pkl','rb'))
            print('Loaded records from the train_pkl file')
            config.image_folder = '../../data/CrowdHuman/CrowdHuman_train/Images'
        else:
            self.training = False
            source = config.eval_source
            self.short_size = config.eval_image_short_size  #800
            self.max_size = config.eval_image_max_size      #1400
            self.records = pickle.load(open('../lib/data/CrHu_val4370.pkl','rb'))
            print('Loaded records from the val_pkl file')
            config.image_folder = '../../data/CrowdHuman/CrowdHuman_val/Images'
    ...
    def load_record(self, record):
        ...
        if self.training:
            ...
            keep = (gtboxes[:, 1]>=0) * (gtboxes[:, 3]>=0)  #remove some invalid bboxes
            ...
            im_info = np.array([0, 0, 1, image_h, image_w, nr_gtboxes], dtype=float)
            # print('Loading gtboxes:', gtboxes.shape, int(im_info[5]), image.shape)
    ...
    def merge_batch(self, data):
        # make images in the same batch have the same dimension 
        # an optimal aspect ratio in a batch should be the median of the aspect ratio list
        images = [it[0] for it in data]
        gt_boxes = [it[1] for it in data]
        im_info = np.array([it[2] for it in data])
        batch_height = int(np.max(im_info[:, 3]))
        batch_width = int(np.max(im_info[:, 4]))
        ...
        ground_truth = []
        for it in gt_boxes:
            gt_padded = np.zeros((self.config.max_boxes_of_image, self.config.nr_box_dim)) # gt_bboxes should have the same size per batch            
            if it.shape[-1] > 5:
                it[:, 0:6] *= scale
                it[:, 8:] *= scale # just for lmks
            else:
                it[:, 0:4] *= scale
        ...
        # print('ground_truth:', ground_truth[:, :, 8:].shape, ground_truth[:, :, 8:])
        # im_info
        im_info[:, 0] = t_height
        im_info[:, 1] = t_width
        im_info[:, 2] = scale
...
class ArbitraryImage(CrowdHuman):
    def __init__(self, config, if_train=False, imagepath=''):
        super().__init__(config, if_train)
...
def flip_boxes(boxes, im_w):
    ...
    for i in range(flip_boxes.shape[0]):
        ...
        if flip_boxes.shape[1] > 10:            
            flip_boxes[i, 8:12:2] = im_w - boxes[i, 10:6:-2] - 1
            flip_boxes[i, 14:18:2] = im_w - boxes[i, 16:12:-2] - 1
            flip_boxes[i, 12] = im_w - boxes[i, 12] - 1
    return flip_boxes
