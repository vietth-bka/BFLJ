...
def bbox_transform_inv_opr_lmk(bbox, deltas):
    # max_delta = math.log(1000.0 / 16)
    """ Transforms the learned deltas to the final bbox coordinates, the axis is 1"""
    bbox_width = bbox[:, 2] - bbox[:, 0] + 1
    bbox_height = bbox[:, 3] - bbox[:, 1] + 1
    ...
    pred_ctr_x = bbox_ctr_x + deltas[:, 0] * bbox_width
    pred_ctr_y = bbox_ctr_y + deltas[:, 1] * bbox_height    
    pred_lmk_x1 = bbox_ctr_x + deltas[:, 2] * bbox_width
    pred_lmk_y1 = bbox_ctr_y + deltas[:, 3] * bbox_height
    pred_lmk_x2 = bbox_ctr_x + deltas[:, 4] * bbox_width
    pred_lmk_y2 = bbox_ctr_y + deltas[:, 5] * bbox_height
    pred_lmk_x3 = bbox_ctr_x + deltas[:, 6] * bbox_width
    pred_lmk_y3 = bbox_ctr_y + deltas[:, 7] * bbox_height
    pred_lmk_x4 = bbox_ctr_x + deltas[:, 8] * bbox_width
    pred_lmk_y4 = bbox_ctr_y + deltas[:, 9] * bbox_height
    pred_lmk_x5 = bbox_ctr_x + deltas[:, 10] * bbox_width
    pred_lmk_y5 = bbox_ctr_y + deltas[:, 11] * bbox_height

    pred_boxes = torch.cat((pred_ctr_x.reshape(-1, 1), pred_ctr_y.reshape(-1, 1),
                            pred_lmk_x1.reshape(-1, 1), pred_lmk_y1.reshape(-1,1),
                            pred_lmk_x2.reshape(-1, 1), pred_lmk_y2.reshape(-1,1),
                            pred_lmk_x3.reshape(-1, 1), pred_lmk_y3.reshape(-1,1),
                            pred_lmk_x4.reshape(-1, 1), pred_lmk_y4.reshape(-1,1),
                            pred_lmk_x5.reshape(-1, 1), pred_lmk_y5.reshape(-1,1)),dim=1)
...
def bbox_transform_opr_lmk(bbox, gt):
    """ Outputs the distance between bbox and gt of center, w, h, head
    and landmarks."""
    bbox_width = bbox[:, 2] - bbox[:, 0] + 1
    bbox_height = bbox[:, 3] - bbox[:, 1] + 1
    bbox_ctr_x = bbox[:, 0] + 0.5 * bbox_width
    ...
    gt_lmk_x1, gt_lmk_y1 = gt[:,8], gt[:,9]
    gt_lmk_x2, gt_lmk_y2 = gt[:,10], gt[:,11]
    gt_lmk_x3, gt_lmk_y3 = gt[:,12], gt[:,13]
    gt_lmk_x4, gt_lmk_y4 = gt[:,14], gt[:,15]
    gt_lmk_x5, gt_lmk_y5 = gt[:,16], gt[:,17]
    
    target_dx1 = (gt_lmk_x1 - bbox_ctr_x) / bbox_width
    target_dy1 = (gt_lmk_y1 - bbox_ctr_y) / bbox_height
    target_dx2 = (gt_lmk_x2 - bbox_ctr_x) / bbox_width
    target_dy2 = (gt_lmk_y2 - bbox_ctr_y) / bbox_height
    target_dx3 = (gt_lmk_x3 - bbox_ctr_x) / bbox_width
    target_dy3 = (gt_lmk_y3 - bbox_ctr_y) / bbox_height
    target_dx4 = (gt_lmk_x4 - bbox_ctr_x) / bbox_width
    target_dy4 = (gt_lmk_y4 - bbox_ctr_y) / bbox_height
    target_dx5 = (gt_lmk_x5 - bbox_ctr_x) / bbox_width
    target_dy5 = (gt_lmk_y5 - bbox_ctr_y) / bbox_height


    target1 = bbox_transform_opr_v2(bbox, gt)
    target = torch.cat([target1, target_dx1.reshape(-1,1), target_dy1.reshape(-1,1),
                                 target_dx2.reshape(-1,1), target_dy2.reshape(-1,1),
                                 target_dx3.reshape(-1,1), target_dy3.reshape(-1,1),
                                 target_dx4.reshape(-1,1), target_dy4.reshape(-1,1),
                                 target_dx5.reshape(-1,1), target_dy5.reshape(-1,1)],
                                 dim=1)
