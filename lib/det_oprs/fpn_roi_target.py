...
from det_oprs.bbox_opr import box_overlap_opr, bbox_transform_opr, box_overlap_ignore_opr, bbox_transform_opr_v2, bbox_transform_opr_lmk, box_overlap_ignore_opr_v2
...
def fpn_roi_target_bfj(rpn_rois, im_info, gt_boxes, top_k=1):
    ...
    # get per image proposals and gt_boxes
    for bid in range(config.train_batch_per_gpu):
        gt_boxes_perimg = gt_boxes[bid, :int(im_info[bid, 5]), :] # the first boxes are true
        batch_inds = torch.ones((gt_boxes_perimg.shape[0], 1)).type_as(gt_boxes_perimg) * bid
        gt_rois = torch.cat([batch_inds, gt_boxes_perimg[:, :4]], axis=1)
        batch_roi_inds = torch.nonzero(rpn_rois[:, 0] == bid, as_tuple=False).flatten()
        all_rois = torch.cat([rpn_rois[batch_roi_inds], gt_rois], axis=0)  #?? concat with gt ???     
        # all_rois = rpn_rois[batch_roi_inds]
        overlaps_normal, overlaps_ignore = box_overlap_ignore_opr_v2(
                all_rois[:, 1:5], gt_boxes_perimg)
        # print('gt_boxes_perimg:', gt_boxes_perimg.shape)
        # print('rpn_rois:', rpn_rois.shape)
        # print('rpn_rois[batch_roi_inds]:', rpn_rois[batch_roi_inds].shape)
        # print('all_rois:', all_rois.shape)
        # exit(-1)
        # print('\noverlaps_normal:', overlaps_normal.shape, ' overlaps_ignore:', overlaps_ignore.shape)
        overlaps_normal, overlaps_normal_indices = overlaps_normal.sort(descending=True, dim=1)
        overlaps_ignore, overlaps_ignore_indices = overlaps_ignore.sort(descending=True, dim=1)
        # gt max and indices, ignore max and indices
        max_overlaps_normal = overlaps_normal[:, :top_k].flatten()      #the 1st column
        gt_assignment_normal = overlaps_normal_indices[:, :top_k].flatten()
        max_overlaps_ignore = overlaps_ignore[:, :top_k].flatten()      #the 1st column
        gt_assignment_ignore = overlaps_ignore_indices[:, :top_k].flatten()
        # cons masks
        ignore_assign_mask = (max_overlaps_normal < config.fg_threshold) * (
                max_overlaps_ignore > max_overlaps_normal)
        ...
        #fg_mask bases on fg_threshold and labels
        fg_mask = (max_overlaps >= config.fg_threshold) * (labels != config.ignore_label)
        #bg_mask bases on bg_threshold
        bg_mask = (max_overlaps < config.bg_threshold_high) * (
                max_overlaps >= config.bg_threshold_low)
        fg_mask = fg_mask.reshape(-1, top_k)
        bg_mask = bg_mask.reshape(-1, top_k)
        # print('fg_mask:', fg_mask.sum(), 'bg_mask:', bg_mask.sum(), (fg_mask*bg_mask).sum())
        pos_max = config.num_rois * config.fg_ratio
        fg_inds_mask = subsample_masks(fg_mask[:, 0], pos_max, True) # randomly remove some True values
        neg_max = config.num_rois - fg_inds_mask.sum()
        bg_inds_mask = subsample_masks(bg_mask[:, 0], neg_max, True)
        # print('fg_inds_mask:', fg_inds_mask.sum(), 'bg_inds_mask:', bg_inds_mask.sum(), (fg_inds_mask + bg_inds_mask).sum())
        labels = labels * fg_mask.flatten()
        keep_mask = fg_inds_mask + bg_inds_mask # select positive boxes and negative boxes
        # labels
        labels = labels.reshape(-1, top_k)[keep_mask]
        ...
        tags = tags.reshape(-1, top_k)[keep_mask] #subsample
        # ious
        ious = max_overlaps * fg_mask.flatten()
        ious = ious.reshape(-1, top_k)[keep_mask] #subsample
        # lmk rois
        lmk_rois = gt_boxes_perimg[gt_assignment, 8:]
        lmk_rois = lmk_rois[keep_mask]        
        # centers
        # ctr_x = all_rois[gt_assignment, 0] + 0.5 * (all_rois[gt_assignment, 2] - all_rois[gt_assignment, 0] + 1) such a stupid indexing
        ...
        gt_assignment = gt_assignment.reshape(-1, top_k)[keep_mask].flatten()
        target_boxes = gt_boxes_perimg[gt_assignment, :]
        rois = all_rois[keep_mask]
        target_rois = rois.repeat(1, top_k).reshape(-1, all_rois.shape[-1])
        # bbox_targets = bbox_transform_opr_v2(target_rois[:, 1:5], target_boxes)
        bbox_targets = bbox_transform_opr_lmk(target_rois[:, 1:5], target_boxes)
        if config.rcnn_bbox_normalize_targets:
            std_opr = torch.tensor(config.bbox_16_normalize_stds[None, :]).type_as(bbox_targets)
            mean_opr = torch.tensor(config.bbox_16_normalize_means[None, :]).type_as(bbox_targets)
            minus_opr = mean_opr / std_opr
            bbox_targets = bbox_targets / std_opr - minus_opr
        bbox_targets = bbox_targets.reshape(-1, top_k * 16)
        return_rois.append(rois)
        return_lmk_rois.append(lmk_rois)
        return_labels.append(labels)
    ...
    if config.train_batch_per_gpu == 1:
        ...
        return_rois = torch.cat(return_rois, axis=0)
        return_lmk_rois = torch.cat(return_lmk_rois, axis=0)
        return_labels = torch.cat(return_labels, axis=0)
        ...
        return return_rois, return_lmk_rois, return_labels, return_tags, return_ious, return_centers, return_bbox_targets
