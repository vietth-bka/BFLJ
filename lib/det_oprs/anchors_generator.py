...
class AnchorGenerator():
    ...
    def __init__(self, base_size=16, ratios=[0.5, 1, 2],
        base_scale=2):
        self.base_size = base_size              #32
        self.base_scale = np.array(base_scale)  #[1]
        self.anchor_ratios = np.array(ratios)   #[1, 2, 3]
    ...
    def get_plane_anchors(self, anchor_scales: np.ndarray):
        ...
        anchor_scales = anchor_scales[None, ...] # [[16]]
    ...
    def get_center_offsets(self, fm_map, stride):
        ...
        # print('size:', fm_height, fm_width)
        f_device = fm_map.device
        shift_x = torch.arange(0, fm_width, device=f_device) * stride #0 -> fm_width, step=1
        shift_y = torch.arange(0, fm_height, device=f_device) * stride
        broad_shift_x = shift_x.reshape(-1, shift_x.shape[0]).repeat(fm_height,1) # fm_height x fm_width
        broad_shift_y = shift_y.reshape(shift_y.shape[0], -1).repeat(1,fm_width) # fm_height x fm_width
        # print(broad_shift_x.shape, broad_shift_y.shape)
    ...
    def __call__(self, featmap, base_stride, off_stride):
        return self.get_anchors_by_feature(featmap, base_stride, off_stride) # 4 16
