...
def find_top_rpn_proposals(is_train, rpn_bbox_offsets_list, rpn_cls_prob_list,
    ...
    box_min_size = config.rpn_min_box_size #2
    bbox_normalize_targets = config.rpn_bbox_normalize_targets #False
    bbox_normalize_means = config.bbox_normalize_means
    bbox_normalize_stds = config.bbox_normalize_stds
    list_size = len(rpn_bbox_offsets_list) #5
    ...
    for bid in range(batch_per_gpu):
        ...
        for l in range(list_size):
            ...
            offsets = rpn_bbox_offsets_list[l][bid].permute(1, 2, 0).reshape(-1, 4) #eg. [2, 12, 208, 272] -> [208, 272, 12] -> [..., 4]
            ...
            if config.anchor_within_border: #False
                proposals = clip_boxes_opr(proposals, im_info[bid, :])
            probs = rpn_cls_prob_list[l][bid] \
                    .permute(1,2,0).reshape(-1, 2) #eg. [2, 6, 208, 272] -> [208, 272, 6] -> [..., 2]
            probs = torch.softmax(probs, dim=-1)[:, 1]
            # gather the proposals and probs
            batch_proposals_list.append(proposals)
            batch_probs_list.append(probs)
        batch_proposals = torch.cat(batch_proposals_list, dim=0) # A x 4
        batch_probs = torch.cat(batch_probs_list, dim=0)         # B x 2
        # filter the zero boxes.
        batch_keep_mask = filter_boxes_opr(
                batch_proposals, box_min_size * im_info[bid, 2]) #Remove all boxes with any side smaller than min_size
        ...
        batch_probs = batch_probs[:num_proposals] # find top num_proposals boxes with highest probs
