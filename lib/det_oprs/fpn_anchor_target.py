    ...
    for bid in range(config.train_batch_per_gpu):
        ...
        for i in range(len(pred_cls_score_list)):
            pred_cls_score_perlvl = pred_cls_score_list[i][bid] \
                .permute(1, 2, 0).reshape(-1, 2)    #eg. [6, 13, 19] -> [13, 19, 6] -> [13x19x3, 2]
            pred_bbox_offsets_perlvl = pred_bbox_offsets_list[i][bid] \
                .permute(1, 2, 0).reshape(-1, 4)    #eg. [12, 13, 19] -> [13, 19, 12] -> [12x19x3, 4]
...
def fpn_anchor_target_opr_core_impl(gt_boxes, im_info, anchors, allow_low_quality_matches=True):
    ...
    # get the gt boxes
    # print('gt_boxes:', gt_boxes.shape, int(im_info[5]))
    valid_gt_boxes = gt_boxes[:int(im_info[5]), :] #nr_gtboxes, gt_boxes: [750, 8]
    valid_gt_boxes = valid_gt_boxes[valid_gt_boxes[:, 7].gt(0)] #notice !!! BIG ERROR if add lmk but keep valid_gt_boxes[:, -1]!!!
    # print('valid_gt_boxes:', valid_gt_boxes.shape)
    # compute the iou matrix
    anchors = anchors.type_as(valid_gt_boxes)
    overlaps = box_overlap_opr(anchors, valid_gt_boxes[:, :4]) #check IOU of each anchor-gt pair [N, M]
    # print('overlaps:', overlaps.shape)
    # match the dtboxes
    max_overlaps, argmax_overlaps = torch.max(overlaps, axis=1) #find closest gt boxes of each anchor box
    # print('argmax_overlaps:', argmax_overlaps.shape)
    #_, gt_argmax_overlaps = torch.max(overlaps, axis=0)
    gt_argmax_overlaps = my_gt_argmax(overlaps)     #find closest anchor of each gt box
    ...
    # set negative ones
    labels = labels * (max_overlaps >= config.rpn_negative_overlap) # max_overlaps >= 0.3
    # set positive ones
    fg_mask = (max_overlaps >= config.rpn_positive_overlap) #rpn_positive_overlap=0.7
    if allow_low_quality_matches:
        gt_id = torch.arange(valid_gt_boxes.shape[0]).type_as(argmax_overlaps) # num gt_boxes
    ...
    # bbox targets
    # print('Check size:', anchors.shape, valid_gt_boxes[argmax_overlaps, :4].shape, valid_gt_boxes.shape)
    bbox_targets = bbox_transform_opr(
            anchors, valid_gt_boxes[argmax_overlaps, :4])   # box displacement 
    if config.rpn_bbox_normalize_targets:   #false
...
def fpn_anchor_target(boxes, im_info, all_anchors_list):
    ...
    for bid in range(config.train_batch_per_gpu):
        ...
        for i in range(len(all_anchors_list)):
            ...
            # print('anchors_perlvl:', anchors_perlvl.shape)
            rpn_labels_perlvl, rpn_bbox_targets_perlvl = fpn_anchor_target_opr_core_impl(
                boxes[bid], im_info[bid], anchors_perlvl) # return label of pairs and displacement between 'em
...
def my_gt_argmax(overlaps):
    ...
    # print('gt_max_mask:', gt_max_mask[:,0].sum())
    a, _ = torch.max(overlaps[:,0], axis=0)
    # print('sum:', (overlaps[:,0]==a).sum())

    gt_argmax_overlaps = []
    for i in range(overlaps.shape[-1]):
        gt_max_inds = torch.nonzero(gt_max_mask[:, i], as_tuple=False).flatten() #get id of nonzero elements        
        gt_max_ind = gt_max_inds[torch.randperm(gt_max_inds.numel(), device=gt_max_inds.device)[0,None]] # numel: number of elements
