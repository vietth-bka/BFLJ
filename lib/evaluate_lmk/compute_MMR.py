...
class COCO_MD:
    def __init__(self, annotation_file=None):
        ...
        if not annotation_file == None:
            ...
            # print(dataset['annotations'][110:120])
            # print('*Check cate:', 'categories' in dataset)
    ...
    def loadRes(self, resFile):
        ...
        print('len res.data:', len(res.dataset['images']))
        print('Loading and preparing results...')
        tic = time.time()
        if type(resFile) == str or type(resFile) == unicode:
            anns = json.load(open(resFile))
            print('Length of anns:', len(anns))
        ...
        # currently, 'annotations' is not in res.dataset
...
class COCOMReval:
    ...
    def evaluateImg(self, imgId, catId, hRng, vRng, f_hRng, maxDet):
        ...
        # print('len gt, dt:', len(gt), len(dt), 'b_ious, f_ious:', b_ious.shape, f_ious.shape)
        # exit(-1)
        # orig method
    ...
    def accumulate(self, p = None):
        ...
        print('len self.evalImgs:', len(self.evalImgs))
        # exit(-1)
        # allows input customized parameters
        if p is None:
            p = self.params
        p.catIds = p.catIds if p.useCats == 1 else [-1]
        T           = len(p.b_iouThrs)  #np.array([0.5])
        R           = len(p.fppiThrs)   #np.array([0.0100,    0.0178,    0.0316,    0.0562,    0.1000,    0.1778,    0.3162,    0.5623,    1.0000])
        K           = len(p.catIds) if p.useCats else 1
        M           = len(p.maxDets)    #[1000]
        ...
        for k, k0 in enumerate(k_list):
            ...
            for m, maxDet in enumerate(m_list):
                ...
                print('dtm:', np.where(dtm<1)[0].shape,
                    'dtIg:', dtIg.shape,
                    'gtIg:', gtIg.shape,
                    'npig', npig,
                    'dtS:', dtScores.shape,
                ...
                tps = tps[:,inds]
                fps = fps[:,inds]
                print('tps:', tps.shape, tps.sum(), 'fps:', fps.shape, fps.sum())

                tp_sum = np.cumsum(tps, axis=1).astype(dtype=np.float)
                print('tp_sum:', tp_sum[:,:5])
                fp_sum = np.cumsum(fps, axis=1).astype(dtype=np.float)
                print('fp_sum:', fp_sum[:,:5])
